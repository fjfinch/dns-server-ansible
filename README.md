...
Note: Make sure your device already obtained an IP address. Internet access is required to install stuff, and the network configurations are used for the Pi-hole container and the netplan task. If you have custom network configs, disable the netplan task. Because this will - either override your config or create conflicts. Just make sure your config has the dns nameserver point to *127.0.0.1*. So that your host also uses Pi-hole directly.
